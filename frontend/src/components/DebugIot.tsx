...
export default function DebugIot() {
  const msgs = useIotLive<TelemetryMsg>(2000);
  ...
  const sampleIds = useMemo(
    () => Array.from(allIdsRef.current).slice(0, 20),
    [total]
  );
    ...
    >
      ...
      >
        <div style={{ fontWeight: 600 }}>
          IoT Debug — Raw Uniques Since Mount
        </div>
        <button onClick={reset} style={{ padding: "4px 8px", fontSize: 12 }}>
          Reset
        </button>
      </div>

      <p style={{ margin: "4px 0 8px" }}>
        Total unique <code>device_id</code>s: <strong>{total}</strong> •
        Elapsed: {elapsedSec}s • ~{ratePerSec}/s
      </p>
